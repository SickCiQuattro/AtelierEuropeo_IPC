                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <small class="text-muted">
                                    ...
                                    Stato:
                                    <span
                                        class="badge bg-{{ $project->status === 'published' ? 'success' : ($project->status === 'draft' ? 'warning' : 'secondary') }}">
                ...
                <div class="col-md-12">
                    <div class="alert alert-info border-left-info" role="alert">
                        <div class="d-flex align-items-center">
                            ...
                            <div>
                                <p class="mb-0">
                                    <strong>Partecipanti approvati:</strong>
                                    {{ $stats['approved'] }}/{{ $project->requested_people }}
                                    @if($project->requested_people - $stats['approved'] > 0)
                                        - Rimangono <strong>{{ $project->requested_people - $stats['approved'] }}</strong> posti
                                        disponibili.
                                    @endif
                                </p>
                            </div>
                                                    ...
                                                    <div class="btn-group" role="group">
                                                        ...
                                                        @if($application->status === 'pending')
                                                            ...
                                                            <button type="button"
                                                                ...
                                                                @if(!$limitReached) data-bs-toggle="modal"
                                                                data-bs-target="#approveModal{{ $application->id }}" @endif
                                                                title="{{ $limitReached ? 'Limite partecipanti raggiunto' : 'Approva' }}"
                                                                {{ $limitReached ? 'disabled' : '' }}>
                                                                <i class="bi bi-check-lg"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-ae btn-ae-outline-danger btn-sm"
                                                                data-bs-toggle="modal"
                                                                data-bs-target="#rejectModal{{ $application->id }}" title="Rifiuta">
                                                                <i class="bi bi-x-lg"></i>
                                                            </button>
                                                        @else
                                                            <button type="button" class="btn btn-ae btn-ae-outline-secondary btn-sm"
                                                                data-bs-toggle="modal"
                    ...
                    <div class="modal-content">
                        <form action="{{ route('admin.applications.approve', $application) }}" method="POST">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="admin_message{{ $application->id }}" class="form-label">Messaggio per il candidato
                                        (opzionale)</label>
                                    <textarea class="form-control" id="admin_message{{ $application->id }}" name="admin_message"
                                        rows="3"
                    ...
                    <div class="modal-content">
                        <form action="{{ route('admin.applications.reject', $application) }}" method="POST">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    <label for="reject_message{{ $application->id }}" class="form-label">Messaggio per il candidato
                                        <span class="text-danger">*</span></label>
                                    <textarea class="form-control" id="reject_message{{ $application->id }}" name="admin_message"
                                        rows="3" required placeholder="Spiega i motivi del rifiuto..."></textarea>
                                    <div class="form-text">È importante spiegare i motivi del rifiuto per aiutare il candidato a
                                        migliorare.</div>
                                </div>
                            </div>
                            <div class="modal-footer">
            ...
            <div class="modal fade" id="updateModal{{ $application->id }}" tabindex="-1">
                <div class="modal-dialog">
                    ...
                    @endphp
                                ...
                                <div class="mb-3">
                                    <label for="update_message{{ $application->id }}" class="form-label">Messaggio per il
                                        candidato</label>
                                    <textarea class="form-control" id="update_message{{ $application->id }}" name="admin_message"
                                        rows="3"
